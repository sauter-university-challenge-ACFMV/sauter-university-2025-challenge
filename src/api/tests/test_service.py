# src/api/tests/test_ons_service.py
...
def test_process_reservoir_data_partial_success(monkeypatch: Any) -> None:
    ...
    async def fake_get(self: Any, url: str, *a: Any, **k: Any): # type: ignore[no-untyped-def]
...
def test_process_reservoir_data_all_success(monkeypatch: Any) -> None:
    ...
    resources = [{"format": "PARQUET", "url": "http://u/2022.parquet", "name": "n 2022"}]
...
def test_download_parquet_data_already_exists(monkeypatch: Any) -> None:
    class FakeRepo:
        def __init__(self) -> None: self.bucket_name = "b"
        def save(self, file: Any, filename: str) -> str: return f"gs://b/{filename}"
        def raw_table_has_value(self, package_name: str, column_name: str, last_day: str) -> bool: return True  # Data exists
...
def test_build_gcs_path_for_future_year(monkeypatch: Any) -> None:
    ...
    # A year far in the future should create a date-partitioned path
    path = s._build_gcs_path("file_9999.csv", resource_year=9999, package_name="pkg")
